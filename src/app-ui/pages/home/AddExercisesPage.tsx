  ...
  const handleSelectedExercise = (exercise: IExercise) => {
    ...
    if (!isSelected) {
      ...
      setSelectedExercises(
        selectedExercises.filter(val => val.eId !== exercise.eId),
      );
    }
  };
  ...
  const addExercises = async () => {
    ...
    try {
      ...
      const response = await postExercises(
        selectedExercises.map(val => val.eId),
        workoutId!,
      );
        ...
        >
          <View style={[styles.exerciseContainer]}>
            ...
            <View
              style={[
                styles.checkbox,
                { backgroundColor: isSelected ? '#6f7896' : 'white' },
              ]}
            >
              {isSelected && (
                <AntDesign name='check' size={15} color='#f6f6f6' />
              )}
            </View>
          </View>
        </Pressable>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <SectionList
        sections={
          text.length > 0
            ? filterDataBySearch(getExerciseSortedList(exercises), text)
            : getExerciseSortedList(exercises)
        }
      ...
      <View style={[styles.footer]}>
        <TouchableOpacity
          style={{
            backgroundColor:
              selectedExercises.length === 0 ? '#494949' : '#B0B0B0',
            borderRadius: 10,
            width: '30%',
          }}
        ...
        >
          <Text
            style={[
              styles.buttonTitle,
              {
                color: selectedExercises.length === 0 ? '#ffffff50' : '#ffffff',
              },
            ]}
          >
            Clear
        ...
        <TouchableOpacity
          style={{
            backgroundColor:
              selectedExercises.length === 0 ? '#4e547e' : '#7F87CD',
            borderRadius: 10,
            width: '60%',
          }}
          disabled={selectedExercises.length === 0}
